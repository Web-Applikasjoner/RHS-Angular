       ...
       public async Task<bool> Create(Item item)
        {
            ...
            {
                _logger.LogError("[ItemRepository] item creation failed for item {@viewModel}, error message: {e}", item, e.Message);
                return false;
            }
        }
        ...
        public async Task<bool> Update(Item item)
        {
            ...
            {
                _logger.LogError("[ItemRepository] Item update failed for ItemId {ItemId:0000}, error message: {e}", item, e.Message);
